...
const SHEET_SETUP_CONFIG = {
  ...
  HEADERS: {
    ...
    'Dashboard': [],
    'Settings': ['Setting Name', 'Setting Value', 'Description']
  },
  ...
  ARRAY_FORMULAS: {
    'Project Pipeline': {
      ...
      'overdue_tasks_count': `=MAP(B2:B, LAMBDA(sfid, IF(sfid="", "", COUNTIFS(Tasks!B:B, sfid, Tasks!D:D, "<>Done", Tasks!G:G, "<"&INT(Settings!$B$2)))))`,
      ...
      'escalate_ready': `=MAP(AC2:AC, LAMBDA(ac, IF(ac="", "", AND(NOT(ISBLANK(ac)),(Settings!$B$2-ac)>=1))))`,
      ...
      'aging_days_since_edit': `=MAP(X2:X, LAMBDA(x, IF(x="", "", IFERROR(INT((Settings!$B$2-x)),0))))`,
      ...
      'blocked_hours': `=MAP(AC2:AC, LAMBDA(ac, IF(ac="", "", 24*(Settings!$B$2-ac))))`,
      // Flags projects that have been aging for 7 days or more since the last edit.
      'staleness_flag': `=MAP(BV2:BV, LAMBDA(bv, IF(bv="", "", bv>=Settings!$B$4)))`
    },
    'Tasks': {
  ...
  STATIC_FORMULAS: {
    ...
    'Dashboard': {
        ...
        'B4': `=COUNTIFS('Project Pipeline'!J:J,"Approved",'Project Pipeline'!R:R,">="&INT(Settings!$B$2),'Project Pipeline'!R:R,"<="&INT(Settings!$B$2)+Settings!$B$3)`,
        'A5': 'Upcoming Revenue (7d)',
        // Sums the weighted revenue for projects scheduled in the next 7 days.
        'B5': `=SUMIFS('Project Pipeline'!BT:BT,'Project Pipeline'!J:J,"Approved",'Project Pipeline'!R:R,">="&INT(Settings!$B$2),'Project Pipeline'!R:R,"<="&INT(Settings!$B$2)+Settings!$B$3)`,
        'A6': 'Framing (7d)',
        // Counts projects in 'Permitting' stage with a deadline in the next 7 days.
        'B6': `=COUNTIFS('Project Pipeline'!I:I,"Permitting",'Project Pipeline'!R:R,">="&INT(Settings!$B$2),'Project Pipeline'!R:R,"<="&INT(Settings!$B$2)+Settings!$B$3)`,
        'A7': 'Overdue Tasks',
        // Counts all tasks that are not done and have a due date in the past.
        'B7': `=COUNTIFS(Tasks!D:D,"<>Done",Tasks!G:G,"<"&INT(Settings!$B$2))`,
        ...
        'B10': `=COUNTIFS('Project Pipeline'!P:P,"Salesforce",'Project Pipeline'!AG:AG,">="&INT(Settings!$B$2)-WEEKDAY(INT(Settings!$B$2),2)+1,'Project Pipeline'!AG:AG,"<"&INT(Settings!$B$2)-WEEKDAY(INT(Settings!$B$2),2)+8)`,
        'A11': 'Forecast (This Month)',
        // Sums the weighted revenue for projects forecasted to close this month.
        'B11': `=SUMIFS('Project Pipeline'!BT:BT,'Project Pipeline'!BO:BO,TEXT(INT(Settings!$B$2),"yyyy-mm"))`,
        'A12': 'Forecast (Next Month)',
        // Sums the weighted revenue for projects forecasted to close next month.
        'B12': `=SUMIFS('Project Pipeline'!BT:BT,'Project Pipeline'!BO:BO,TEXT(EDATE(INT(Settings!$B$2),1),"yyyy-mm"))`,
